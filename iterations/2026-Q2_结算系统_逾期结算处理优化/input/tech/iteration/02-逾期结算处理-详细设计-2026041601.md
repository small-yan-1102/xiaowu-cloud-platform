...
4. `[新增]` 对查询到的每条 overdueRecord，逐条执行逾期判定：
   ...
   4. 如果 publishedAt 为空：跳过该条记录，记录警告日志：`log.warn("视频发布日期为空，跳过逾期判定, videoId={}", videoId)`
   5. 如果 registrationDate 为空：跳过该条记录，记录警告日志：`log.warn("视频登记日期为空，跳过逾期判定, videoId={}", videoId)`
...
```sql
-- 1. 修复死锁数据：将历史已登记但未逾期的 status=0 记录修复为 status=3
-- 判定条件：registration_time IS NOT NULL AND registration_time <= 发布次月28日
-- 注意：此脚本需要结合 video_composition 表的 published_at 计算，建议在应用层编写
-- 伪SQL示意：
...
WHERE vco.deleted = 0
  ...
  AND vco.registration_time <= DATE_FORMAT(
