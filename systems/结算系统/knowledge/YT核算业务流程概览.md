...
> **逾期结算处理分支说明**：运营将无归属视频ID导入到逾期结算处理模块（SET-03），系统按视频ID查询冲销报表中的无归属行，**直接使用冲销报表已有的 `source_channel_revenue_ratio` 等占比字段**（非实时计算），执行 R1~R5 占比校验和误差抹平后，生成【未登记】收益记录。详见 `逾期结算处理业务逻辑.md` §5.3。
