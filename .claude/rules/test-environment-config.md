---
trigger: always_on
description: 测试环境基础设施配置 - 数据库连接、服务器信息、系统登录账号、通用登录路径（AI执行测试的前置必读信息）
---
...
| 服务器 | IP | 账号 | 密码 | 角色 |
|--------|-----|------|------|------|
| 服务器 A | 172.16.24.200 | test | wgu4&Q_2 | 前端 + 部分后端服务 |
| 服务器 B | 172.16.24.204 | test | wgu4&Q_2 | 后端服务 |
| 服务器 B (root) | 172.16.24.204 | root | 7}SZ@kRIY+^.-clyZw | 运维操作时使用 |
...
|---------|---------|------|---------|---------|------|
| 主分销商 | Yancz-cool@outlook.com | TxdvZ06y | ALL（全量数据） | HELLO BEAR (team_id=1988839584685428736) | 冒烟测试主账号，MQ 同步验证 |
| 权限受限 | 15057199668 | 4aYkEmKJ | 部门级（仅本部门） | YUJA-001 (team_id=1985522863929118720) | 权限隔离测试 |
| 空数据 | 18506850780 | 1KXuVhmT | SCOPE（范围管理） | YUJA-001 | 空列表/无数据场景测试 |
| 第二分销商 | 17835727272 | Jlb123456 | ALL（全量数据） | team_id=1988520080772243456 | 多分销商交叉验证 |

**登录路径**：
```
1. 访问 http://distribute.test.xiaowutw.com/login
2. 在账号输入框输入邮箱或手机号（如 Yancz-cool@outlook.com）
3. 在密码输入框输入对应密码（如 TxdvZ06y）
4. 点击「登录」按钮
...
| 账号标识 | 手机号 | 验证码 | 用途 |
|---------|--------|--------|------|
| 主账号 | 15139407453 | 111111 | 功能测试主账号 |

**登录路径**：
```
1. 访问 http://dubbing-1.test.xiaowutw.com/login
2. 选择「验证码登录」Tab
3. 输入手机号 15139407453，验证码 111111
4. 点击登录，进入首页
5. 左侧导航：首页、视频译制、系统设置
```
...
**请求示例**：
```bash
curl 'http://172.16.24.200:8011/sso/doLogin?name=18202748232&pwd=1111' \
  -X 'POST' \
...
```json
{
  ...
  "data": {
    "token": "eyJ...(JWT)...",
    "userInfo": {
      "userId": "17483421285619335",
      "name": "用户名",
      "deptIdList": ["978402599"]
    }
  }
}
```
...
- 验证码有效期 10 分钟
- 测试环境固定验证码 `1111`（4位）或 `111111`（6位，剧译宝）
...
>
> 文件将创建于：`systems/CRM/knowledge/data-testid/`

### 6.6 总控系统

> **data-testid 尚未创建**。总控系统测试用例暂无 data-testid 映射文件，元素定位退回使用可见文本 → aria-label → CSS 选择器顺序。
>
> 文件将创建于：`systems/总控系统/knowledge/data-testid/`
...
5. **新增系统**：后续新系统的 data-testid 文件同样添加到本索引中，并将 §6.5/6.6 的"尚未创建"占位条目更新为实际文件列表
