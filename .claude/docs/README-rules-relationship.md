...
| **v3.8** | 新增 AI 测试能力边界定义：§4.2 断言类型分类（视觉/API/DB）、§4.6 确定性断言编写规范、B.2 人工介入三级分级（Tier-1 可 Mock / Tier-2 必须人工 / Tier-3 专用工具）、E.7 静态数据池规范、§3.4 自愈边界规则（环境层 vs 业务层）、§5.6 确定性断言执行规则 | 8 个规则文件 + 2 个模板 + 7 个 Skill |
| **v3.9（当前）** | 规则文件分层：7 个规则文件从 `.claude/rules/` 迁移至 `.claude/docs/`（按需读取，不再自动注入 context）；`rules/` 仅保留 `rules.md` + `test-environment-config.md`（~24KB）；敏感凭证分离至 `.claude/secrets/`；13 个 Skill 支撑文件路径修复（assets/references 子目录）；CLAUDE.md 精简 56% | 2 个自动加载 + 7 个按需读取 + 43 个 Skill |

### 当前状态

#### AI 直接执行模式（功能测试/冒烟测试/线上回归）

| 文件 | 当前版本 | 职责 | 存放位置 | 加载方式 |
|------|---------|------|---------|---------|
| `rules.md` | v2.0 | 基础规则（语言、环境、角色、执行模式定义） | `.claude/rules/` | 自动加载 |
| `test-environment-config.md` | — | 测试环境 URL、登录路径、data-testid 索引（已脱敏） | `.claude/rules/` | 自动加载 |
| `ai_test_case_design.md` | v3.6 | 测试用例设计规范（附录 A-G、确定性断言、人工介入分级、静态数据池） | `.claude/docs/` | 按需读取 |
| `smoke_test_design.md` | v2.0 | 冒烟测试设计规范 | `.claude/docs/` | 按需读取 |
| `ai_execution_strategy.md` | v2.2 | AI执行策略（自愈边界、确定性断言执行、数据就绪验证、生产环境约束） | `.claude/docs/` | 按需读取 |
| `ai_execution_report.md` | v1.2 | AI执行报告（结果记录、截图规范、汇总报告、线上回归报告） | `.claude/docs/` | 按需读取 |
| `tech_doc_review.md` | v2.3 | 技术文档审阅检查标准 | `.claude/docs/` | 按需读取 |

#### 技术优化项目测试（无PRD的技术重构/优化）

| 文件 | 当前版本 | 职责 | 存放位置 | 加载方式 |
|------|---------|------|---------|---------|
| `tech-optimization-ai-testing-rules.md` | v1.0 | 准入规则（素材链、开发交付物、职责分工、准入检查） | `.claude/docs/` | 手动引用 |
| `tech-optimization-ai-testing-guide.md` | v1.0 | 使用说明（6 步操作指南、文件构成与角色关系） | `.claude/docs/` | 手动引用 |
| `templates/tech-optimization-ai-test-skill-template.md` | v1.0 | AI Skill 模板（6 种工作模式） | `.claude/docs/templates/` | 手动引用 |
| `templates/tech-optimization-test-cases-template.md` | v1.0 | 测试用例模板（6 阶段结构） | `.claude/docs/templates/` | 手动引用 |
...
> **v3.9 变更**：规则文件已从 `.claude/rules/` 迁移至 `.claude/docs/`，不再自动注入 context。
> 仅 `rules.md` 和 `test-environment-config.md` 保留在 `.claude/rules/`（自动加载）。
> 各 Skill 通过本地 `references/` + `assets/` 文件按需加载所需规则内容。

| 文件 | 定位层级 | 核心职责 | 存放位置 | 加载方式 |
|------|---------|---------|---------|---------|
| `rules.md` | **基础层** | 对话语言、系统环境、角色定义、执行模式 | `rules/` | 自动加载 |
| `test-environment-config.md` | **基础层** | 环境 URL、登录路径、data-testid 索引 | `rules/` | 自动加载 |
| `ai_test_case_design.md` | **设计层** | 测试用例设计原则、场景覆盖、验证判断规范、线上回归适配规则 | `docs/` | 按需读取 |
| `smoke_test_design.md` | **设计层-专项** | 冒烟测试选取标准、设计原则、验证层级 | `docs/` | 按需读取 |
| `ai_execution_strategy.md` | **执行层** | 浏览器操作、等待、重试、部署预检、生产环境约束、线上回归流程 | `docs/` | 按需读取 |
| `ai_execution_report.md` | **报告层** | 执行结果记录、截图规范、汇总报告、线上回归报告 | `docs/` | 按需读取 |
| `tech_doc_review.md` | **审阅层** | 技术文档审阅检查标准（需求一致性、风险场景、AI可测试性） | `docs/` | 按需读取 |
| `tech-optimization-ai-testing-rules.md` | **专项-技术优化** | 技术优化项目准入规则 | `docs/` | 手动引用 |
| `tech-optimization-ai-testing-guide.md` | **专项-技术优化** | 技术优化AI测试使用说明 | `docs/` | 手动引用 |
...
**存放位置**: `.claude/docs/`（按需读取，不再自动注入）

**原 glob 匹配规则**（迁移前）: `**/md_case/*.md`、`**/test_case*.md`、`**/*测试用例*.md`、`**/*test_plan*.md`
...
**原 glob 匹配规则**（迁移前）: `**/md_case/*smoke*`、`**/*冒烟*.md`、`**/*smoke*.md`
...
**原 glob 匹配规则**（迁移前）: `**/md_case/*.md`、`**/test_case*.md`、`**/*测试用例*.md`、`**/*smoke*.md`、`**/*冒烟*.md`
...
**原 glob 匹配规则**（迁移前）: `**/md_case/*.md`、`**/test_case*.md`、`**/*测试用例*.md`、`**/*测试报告*.md`、`**/*test_report*.md`、`**/*执行结果*.md`
...
**存放位置**: `.claude/docs/`（手动引用）（适用于无 PRD 的技术优化项目，不自动触发）
...
D2 技术文档审阅
    └── Skill: tech-doc-review
            ├── Ref: .claude/docs/tech_doc_review.md v2.3（检查标准）
            ├── 以 D1 PRD 审阅报告为参照基准
...
D3 测试用例设计
    └── Skill: test-case-design
            ├── Mode A: 基于 PRD/流程图设计
            │       └── Ref: .claude/docs/ai_test_case_design.md v3.6
            ├── Mode B: 基于 D2 审阅报告补充
            │       └── 审阅报告 §二.2.3 作为输入
            ├── Mode C: 线上回归用例设计
            │       ├── Ref: .claude/docs/ai_test_case_design.md G.3.4 + G.3.5
            │       ├── Ref: .claude/docs/ai_execution_strategy.md §8
            │       └── 输出 suite_prod_regression.md
            └── 冒烟用例
                    └── Ref: .claude/docs/smoke_test_design.md v2.0

D4 冒烟 + 测试执行（测试环境）
    └── Ref: .claude/docs/ai_execution_strategy.md v2.2
            ├── §4.5 部署预检（环境可访问、登录正常、页面可加载）
            ├── 冒烟测试（失败则打回开发）
            ├── E2E → 异常 → 边界
            ├── §5.1 文本匹配以技术文档标注值为准（U-8 互引）
            └── Ref: .claude/docs/ai_execution_report.md v1.2
...
| 技术文档审阅 | Skill: `tech-doc-review` → Ref: `.claude/docs/tech_doc_review.md` |
| 设计测试用例 | Skill: `test-case-design` → Ref: `.claude/docs/ai_test_case_design.md` |
| 设计冒烟用例 | Skill: `test-case-design` Mode D → Ref: `.claude/docs/smoke_test_design.md` |
| 设计线上回归用例 | Skill: `test-case-design` Mode C → Ref: `.claude/docs/ai_test_case_design.md` G.3.4/G.3.5 |
| 部署预检 | Ref: `.claude/docs/ai_execution_strategy.md` §4.5 |
| 执行测试 | Skill: `test-execution` → Ref: `.claude/docs/ai_execution_strategy.md` |
| 记录结果/生成报告 | Ref: `.claude/docs/ai_execution_report.md` |
| 线上回归执行 | Ref: `.claude/docs/ai_execution_strategy.md` §8/§9 |
| 线上回归报告 | Ref: `.claude/docs/ai_execution_report.md` §6 |
| 结果同步到 Excel | Skill: `test-result-sync` |
| 技术优化项目准入 | Ref: `.claude/docs/tech-optimization-ai-testing-rules.md` |
| 技术优化项目操作 | Ref: `.claude/docs/tech-optimization-ai-testing-guide.md` |
...
> **v3.9 变更**：规则文件迁移后，原 `model_decision` trigger 已失效（文件不在 `rules/` 目录）。
> 各 Skill 通过本地 `references/` 文件加载所需规则内容，不再依赖 context 自动注入。

| 加载方式 | 含义 | 文件 | 存放位置 |
|---------|------|------|---------|
| **自动加载** | 每次对话注入 system prompt | `rules.md`、`test-environment-config.md` | `.claude/rules/` |
| **按需读取** | Skill 执行时通过 Read 加载 | `ai_test_case_design.md`、`smoke_test_design.md`、`ai_execution_strategy.md`、`ai_execution_report.md`、`tech_doc_review.md` | `.claude/docs/` |
| **手动引用** | 用户主动触发时读取 | `tech-optimization-*.md`、`templates/*` | `.claude/docs/` |
...
├─────────────────────────────────────────┤
│  [rules/] 基础层（自动加载）              │
│  rules.md + test-environment-config.md  │
└─────────────────────────────────────────┘
