...
1. **检查配置文件**：读取 `config.yaml`（默认路径 `.claude/skills/yunxiao-sync/config.yaml`）
   - 存在 → 加载配置，跳到 Step 3
   - 不存在 → 进入 Step 2 引导配置
2. **引导配置**（AskUserQuestion）：云效 PAT、Organization ID、Project ID（每次确认）、Sprint ID（可选）
3. **验证配置有效性**：调用云效 API 验证 PAT 和 Project ID 是否有效
   - 有效 → 继续
   - 无效 → 提示用户修正

### Phase 1: 模拟运行（Dry-Run）

1. **扫描测试用例文件**：在用户指定目录（或默认 `testcase/`）下查找 `suite_*.md` 文件
2. **解析用例结构**：提取模块、用例编号、用例名称
3. **对比云效已有数据**：调用 API 查询目标 Testhub 中已有用例
4. **生成差异预览**：新增 {N} 条 / 更新 {N} 条 / 无变化 {N} 条
5. **向用户展示差异**，等待确认

### Phase 2: 执行同步

1. **用户确认后执行**：按差异预览逐条同步
2. **失败处理**：单条失败不阻塞，记录失败列表，继续同步其余
3. **进度展示**：实时显示同步进度（已完成/总数）

### Phase 3: 输出同步报告

输出格式：
- 新增：{N} 条 / 更新：{N} 条 / 失败：{N} 条 / 跳过：{N} 条
- 失败详情表（用例编号 + 用例名称 + 失败原因）
