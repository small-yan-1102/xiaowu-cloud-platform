...
description: 系统功能梳理与需求分析。按输入类型自动路由 4 种模式 — ①输入代码→梳理功能清单(function-inventory)；②输入已有文档→评估颗粒度(granularity-evaluation)；③输入全新 PRD→结构化需求分析(requirement-analysis)；④输入改造 PRD→变更影响分析(change-impact-analysis)。
version: 1.1.0
triggers:
  - "梳理系统功能"
  - "梳理功能清单"
...
> **Mode C / D 区分**：若用户提供 PRD 且涉及现有系统，按以下决策树判断：
> - PRD 中**所有功能均为全新模块**（现有系统无对应功能）→ Mode C
> - PRD 中**部分或全部功能涉及改造/优化现有功能**（含关键词：修改、优化、调整、改造、重构）→ Mode D
> - 无法判断 → 询问用户「PRD 中的功能是全新开发，还是对现有功能的改造？」
