...
>
> Chrome DevTools MCP 不可用 → **降级为文本输入模式**：
> - 提示用户："Chrome DevTools MCP 未安装或不可用。请手动粘贴 Bug 页面的完整内容（包括标题、描述、附件链接等），我将基于文本内容生成分析报告。"
> - 跳过自动页面提取步骤，直接进入信息结构化和分析阶段
> - 在报告中标注"数据来源: 用户手动提供（非自动提取）"
>
>      - **结构化输入模板**（降级模式下提示用户按以下格式粘贴）：
>        ```
>        【标题】：
>        【环境】：测试/预发/生产
>        【复现步骤】：
